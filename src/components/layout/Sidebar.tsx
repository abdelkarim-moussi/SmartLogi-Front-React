    ...
    return (
      ...
      `}
        >
            ...
            {/* User Info */}
            <div className={`px-4 py-4 border-b border-slate-700/50 ${isCollapsed ? 'text-center' : ''}`}>
            ...
            <nav className="p-4 space-y-1 overflow-y-auto h-[calc(100vh-180px)]">
                {navItems.map((item) => (
                    <NavLink
                        key={item.path}
                        to={item.path}
                        end={item.path === '/admin' || item.path === '/manager' || item.path === '/client' || item.path === '/delivery'}
                        className={({ isActive }) => `
...
function TruckIcon({ className = 'w-5 h-5' }: { className?: string }) {
    return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 17h8M8 17a2 2 0 11-4 0 2 2 0 014 0zm8 0a2 2 0 102 0h-2zm-8 0h2m6 0h2M3 13V5a2 2 0 012-2h6l4 4h4a2 2 0 012 2v4m-6 0h.01" />
        </svg>
    );
}
